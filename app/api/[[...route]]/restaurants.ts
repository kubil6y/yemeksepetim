...
import { count, desc, eq, inArray, and, gte, sql } from "drizzle-orm";
    ...
    .get(
        ...
        async (c) => {
            ...
            const data = await db
                ...
                .from(restaurants)
                .innerJoin(foods, eq(foods.restaurantId, restaurants.id))
                .innerJoin(
                    restaurantReviews,
                    eq(restaurantReviews.restaurantId, restaurants.id)
                )
                .where(
                    and(
                        f.getCategories()
                ...
                .groupBy(foods.id, restaurants.id)
                .orderBy(
                    f.filters.sort === "suggested"
                        ? desc(foods.price)
                        : sql`AVG(${restaurantReviews.score}) DESC`
                )
